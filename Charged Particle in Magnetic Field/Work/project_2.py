...
def F_lorentz(v, B, q = constant["e"], z_1 = lens["z_1"], z_2 = lens["L"], R = lens["R"], B_0=lens["B_0"]):
    """
...
def f(t, y, z_1 = lens["z_1"], z_2 = lens["L"], R = lens["R"], B_0=lens["B_0"]):
        # y = [x, y, z, vx, vy, vz]
        pos = y[:3] 
        v = y[3:]
        b = B(pos) # returns array [Bx, By, Bz]
        
        a = F_lorentz(v, b)/constant["m_e"] #returns [ax, ay, az]

        return np.array([y[3], y[4], y[5],
                         a[0], a[1], a[2]])

def rk4(y, f, t, h, z_1 = lens["z_1"], z_2 = lens["L"], R = lens["R"], B_0=lens["B_0"]):
    """Runge-Kutta RK4"""
    k1 = f(t, y)
